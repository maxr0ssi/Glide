...
def draw_info(
    image: np.ndarray,
    det: HandDet | None,
    poses: PoseFlags | None,
    fps: float = 0.0,
    touch_threshold: float = 20.0,
    touch_signals: TouchProofSignals | None = None,
) -> None:
    ...
    if touch_signals is not None:
        ...
        cv2.circle(
            image, (circle_x, circle_y), circle_radius + 2, (255, 255, 255), 2
        )  # White border
        ...
        cv2.line(
            image,
            (table_x + 10, title_y + 5),
            (table_x + table_width - 10, title_y + 5),
            (200, 200, 200),
            1,
        )
        ...
        for label, value in signals:
            if value is not None:
                ...
                value_color = (
                    (0, 255, 0) if value > 0.7 else (0, 255, 255) if value > 0.4 else (0, 100, 255)
                )
                ...
                cv2.rectangle(
                    image, (bar_x, bar_y), (bar_x + bar_width, bar_y + 3), value_color, -1
                )
                cv2.rectangle(image, (bar_x, bar_y), (bar_x + 100, bar_y + 3), (100, 100, 100), 1)
        ...
        # Background
        cv2.rectangle(
            image, (bar_x, bar_y), (bar_x + bar_width, bar_y + bar_height), (50, 50, 50), -1
        )
        ...
        dist_color = (
            (0, 255, 0)
            if touch_signals.distance_factor < 0.3
            else (0, 255, 255) if touch_signals.distance_factor < 0.7 else (0, 100, 255)
        )
        cv2.rectangle(
            image, (bar_x, bar_y), (bar_x + dist_pixels, bar_y + bar_height), dist_color, -1
        )

        # Labels
        cv2.putText(
            image, "Far", (bar_x, bar_y - 3), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (150, 150, 150), 1
        )
        cv2.putText(
            image,
            "Close",
            (bar_x + bar_width - 25, bar_y - 3),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.3,
            (150, 150, 150),
            1,
    ...
    if det is not None:
        ...
        cv2.putText(image, status, (10, height - 70), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        ...
        if poses is not None:
            ...
            if poses.two_up:
                pose_text += "Two Up "
            cv2.putText(
                image, pose_text, (10, height - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2
            )
        ...
        cv2.putText(
            image, touch_text, (width // 2 - 80, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, touch_color, 3
        )
        ...
        cv2.putText(
            image,
            f"Threshold: {touch_threshold:.0f} px",
            (width // 2 - 80, 90),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (255, 255, 255),

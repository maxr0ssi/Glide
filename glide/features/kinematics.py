...
class HandKinematics:
    ...
    middle_tip_rel: tuple[float, float] | None
    finger_length_idx: float
    finger_length_mid: float | None
...
class KinematicsTracker:
    def __init__(self, ema_alpha: float = 0.35, buffer_frames: int = 24) -> None:
        ...
        self._idx_tip_ema: tuple[float, float] | None = None
        self._mid_tip_ema: tuple[float, float] | None = None
        self.trail: deque[tuple[float, float]] = deque(maxlen=buffer_frames)
        self.trail_mid: deque[tuple[float, float]] = deque(
            maxlen=buffer_frames
        )  # Middle finger trail
        self.trail_mean: deque[tuple[float, float]] = deque(
            maxlen=buffer_frames
        )  # Mean of both fingers
    ...
    def compute(self, landmarks: list[Landmark]) -> HandKinematics | None:
    ...
    @staticmethod
    def _ema(
        prev: tuple[float, float] | None, cur: tuple[float, float], alpha: float
    ) -> tuple[float, float]:
        if prev is None:
            return cur
        return (alpha * cur[0] + (1 - alpha) * prev[0], alpha * cur[1] + (1 - alpha) * prev[1])

    def get_mean_fingertip(self) -> tuple[float, float] | None:
    ...
    def get_finger_speeds(self, lookback: int = 1) -> tuple[float | None, float | None]:

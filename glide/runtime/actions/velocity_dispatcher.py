...
class VelocityScrollDispatcher:
    ...
    def __init__(self, config: ScrollConfig, ws_broadcaster: WebSocketBroadcaster | None = None):
    ...
    def dispatch(self, velocity: Vec2D, state: GestureState, is_active: bool) -> bool:
        ...
        if state == GestureState.SCROLLING and is_active:
            # Continuing gesture
            ...
            if self.ws_broadcaster:
                ...
                logger.debug(
                    f"Dispatching scroll to WebSocket: vy={velocity.y}, speed={speed_normalized}"
                )
                self.ws_broadcaster.publish_scroll(velocity.y, speed_normalized)

            return True

        if state == GestureState.IDLE and self.last_state == GestureState.SCROLLING:
            # Ending gesture
